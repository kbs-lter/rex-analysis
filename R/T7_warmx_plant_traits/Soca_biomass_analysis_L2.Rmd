...
# https://www.youtube.com/watch?v=RGn07Dcrkgw
m1_eff <- emmeans(m1, spec = "Climate_Treatment")
summary(m1_eff)
m2_eff <- update(m1_eff, tran=make.tran('power', p=0))
summary(m2_eff, type="response")

mg_eff <- emmeans(m1, spec = "Galling_Status")
summary(mg_eff)
mg2_eff <- update(mg_eff, tran=make.tran('power', p=0))
summary(mg2_eff, type="response")

# Pairwise comparisons for climate treatments
# back-transforming - A vs. W and A vs. WD
exp(1.30039)-exp(1.30039-0.34404) # 1.068547 - warmed plants weigh 1.1 g more than ambient

# Pairwise comparisons for non-galled vs galled
# re-set the reference level for diff. comparisons
biomass <- within(biomass, Galling_Status <- relevel(factor(Galling_Status), ref = "Non-Galled"))
# rerun model
m1 <- lmer(log_biomass ~ Climate_Treatment * Galling_Status + (1|Rep/Footprint/Subplot) + (1|Year), data = biomass, REML=F)
summary(m1)
anova(m1)

# back-transforming - non-galled vs. galled
exp(1.30039)-exp(1.30039-0.08147) # 0.2871964 - galled plants weighed 0.29 g more than non-galled plants
...
estimate <- 0.13  # Estimate
SE <- 0.041      # Standard Error
df <- 422        # Degrees of Freedom (use the value from your model output)

# Find the critical t-value for 95% CI
t_value <- qt(0.975, df)  # For a two-tailed test and alpha = 0.05
...
# Back-transform using the exponential function
estimate_original <- exp(estimate)
lower_ci_original <- exp(lower_ci)
upper_ci_original <- exp(upper_ci)

# Print the results
estimate_original  # This should be around 1.139
lower_ci_original  # This should be around 1.049
upper_ci_original  # This should be around 1.236
```


```{r}
m2 <- lmer(log_biomass ~ Climate_Treatment + Galling_Status + (1|Rep/Footprint/Subplot) + (1|Year), data = biomass, REML=F)
...
contrast(emmeans(m2, ~Galling_Status), "pairwise", simple = "each", combine = F, adjust = "mvt")
```
