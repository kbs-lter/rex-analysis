...
ggplot(green2, aes(x = treatment, y = greenness, fill = gall_present)) +
  ...
  labs(x = "Treatment", y = "Greenness level", fill = "Gall Presence") +
...
# Gall average plot
green_gall <- green2 %>%
  group_by(rep, footprint, plant_num, gall_present) %>%
  summarize(greenness = mean(greenness, na.rm = TRUE))
ggplot(green_gall, aes(x = gall_present, y = greenness, fill = gall_present)) +
  geom_jitter(shape=16, position=position_jitterdodge(), alpha = 0.6, aes(colour = gall_present)) +
