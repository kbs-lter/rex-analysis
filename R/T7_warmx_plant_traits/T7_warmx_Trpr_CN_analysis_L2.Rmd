...
ggplot(alive, aes(x = Nitrogen, y = Weight..mg., group = Subplot_Descriptions, color = Subplot_Descriptions)) +
   geom_point(size = 4) +
...
coefficient <- cor.test(alive$Weight..mg., alive$Nitrogen)
coefficient$estimate
 
#Regression Plot
png("rex_regression_biomass_nitrogen_2022.png", units="in", width=8, height=6, res=300)
ggplot(alive, aes(x = Nitrogen, y = Weight..mg., color = Subplot_Descriptions)) + 
         geom_point(size = 3) +
...
#Model
model1=lmer(Nitrogen ~ Weight..mg. + Subplot_Descriptions + (1|Rep/Footprint_Location), data = alive)
plot(model1)
summary(model1)
anova(model1)
...
fit1 <- lm(Nitrogen ~ Weight..mg. + Subplot_Descriptions + (1|Rep/Footprint_Location), data = alive)
...
m_plot1 <- lmer(Nitrogen ~ Subplot_Descriptions + (1|Rep/Footprint_Location), data=trpr_cn, REML=F)
hist(resid(m_plot1))
shapiro.test(resid(m_plot1))
summary(m_plot1)
anova(m_plot1)
...
```{r}
#Correlation coefficient 
coefficient <- cor.test(trpr_cn$Weight..mg., trpr_cn$Nitrogen)
coefficient$estimate
 
#Regression Plot
png("rex_regression_biomass_nitrogen_2022.png", units="in", width=8, height=6, res=300)
ggplot(trpr_cn, aes(x = Nitrogen, y = Weight..mg., color = Subplot_Descriptions)) + 
         geom_point(size = 3) +
         stat_smooth(method = "lm", col = "red") +
         xlab("Nitrogen Content (%)") +
         ylab(bquote("2022 Red Clover Avg Alive Biomass" ('g/0.20m'^2))) +
...
#Model
model1=lmer(Nitrogen ~ Weight..mg. + Subplot_Descriptions + (1|Rep/Footprint_Location), data = trpr_cn)
plot(model1)
summary(model1)
anova(model1)
emmeans(model1, list(pairwise ~ Subplot_Descriptions), adjust = "tukey")
...
fit1 <- lm(Nitrogen ~ Weight..mg. + Subplot_Descriptions + (1|Rep/Footprint_Location), data = trpr_cn)
ggplotRegression(fit1) # no correlation between soil moisture and Trfpr biomass
```
