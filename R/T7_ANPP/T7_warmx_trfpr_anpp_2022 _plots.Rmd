...
qqnorm(resid(m2))
shapiro.test(resid(m2)) # homogeneity of variance? yes if p >0.05 (p=0.75)
leveneTest(resid(m2) ~ Subplot_Descriptions, data = trfpr_anpp_noinsect) # p=0.36
...
trfpr_plot <- trfpr_anpp_noinsect %>%
  group_by(Rep, Footprint_Location, Subplot_Location, Subplot_Descriptions) %>% 
  summarize(sum_biomass = sum(Biomass_g, na.rm = TRUE))
hist(trfpr_plot$sum_biomass) #right skewed 

# models
m_plot1 <- lmer(sum_biomass ~ Subplot_Descriptions + (1|Rep/Footprint_Location), data=trfpr_plot, REML=F)
anova(m_plot1) #F value = 1.64
