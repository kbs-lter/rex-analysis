...
# TO DO: need to go back to the clean HOBO script to include 2023 temp data
```{r}
...
```{r}
# plot - monthly
png("rex_airtemp_all_years.png", units="in", width=8, height=6, res=300)
ggplot(avg_airtemp2, aes(x = month_year, y = avg_temp, group = Treatment, color = Treatment)) +
        geom_errorbar(aes(ymin=avg_temp-se_temp, ymax=avg_temp+se_temp),width=0.1,color="black",linetype="solid") +
        geom_line(linewidth = 1) +
        facet_grid(~year) +
        scale_color_manual(name="Treatment", 
