...
Average monthly air temperature plot from April - August during daytime
```{r}
# plot - monthly
...
```{r}
png("rex_airtemp_drought_allyears.png", units="in", width=6, height=6, res=300)
...
Wants growing season day time average air temps for 2021 and 2022
```{r}
# create new dataframe with only data from 7 AM - 7 PM 
day.airtemp <- airtemp
...
avg.day.airtemp.2122 <- day.airtemp.2122 %>%
  group_by(year, Treatment, month) %>%
  summarise(avg_temp = mean(Temperature_C, na.rm = TRUE),
            se_temp = std.error(Temperature_C, na.rm = TRUE))

avg.day.airtemp.2122.plot <- 
        ggplot(avg.day.airtemp.2122, aes(x = month, y = avg_temp, group = Treatment, color = Treatment)) +
        geom_errorbar(aes(ymin=avg_temp-se_temp, ymax=avg_temp+se_temp),width=0.1,color="black",linetype="solid") +
        geom_line(linewidth = 1) +
        facet_grid(~year) +
        scale_color_manual(name="Treatment", 
...
day.airtemp.22 <- day.airtemp.2122 %>% filter(year == 2022)

# take average per year by treatment
avg.day.airtemp.22 <- day.airtemp.22 %>%
  group_by(year, Treatment) %>%
  summarise(avg_temp = mean(Temperature_C, na.rm = TRUE),
            se_temp = std.error(Temperature_C, na.rm = TRUE))

ggplot(avg.day.airtemp.22, aes(x = year, y = avg_temp, fill = Treatment)) +
        geom_pointrange(aes(ymin=avg_temp-se_temp, ymax=avg_temp+se_temp), pch = 21,size = 1.2,
                        position=position_dodge(0.5)) +
        #facet_grid(~year) +
        scale_fill_manual(name="Treatment", 
