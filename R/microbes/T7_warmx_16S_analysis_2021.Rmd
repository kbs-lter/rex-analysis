...
Read QZA files into dataframe and re-upload them as .csv files
```{r}
# Code below you only need to do once so it's # out as to not have to run every time
# the qza files are found in the qiime2 artifacts folder in the L1 folder in the REX data microbes folder and also on my hpcc account
#SVs16S <- read_qza("/Users/moriahyoung/Downloads/16S-2021-merged-dada2table.qza")
#SVs16Stable <- SVs16S$data
#write.csv(SVs16Stable, file = "/Users/moriahyoung/Downloads/16S-merged-dada2table.csv") # I think I renamed this to "16S-merged-table.csv"
...
Re-format taxonomic tables & filter data
```{r}
...
# uploaded these to REX google shared drive
```

# Create phyloseq object with filtered data
```{r}
# read in phylogenetic tree qza 
...
Order sample data
Don't do the below code if you're working on differential abundance
```{r}
...
# no irrigated control for balanced design in adonis2 restricted permutations
data_16S_filtered_no_insect_noIRC <- subset_samples(data_16S_filtered_noinsect, Subplot_Descriptions%in%c("ambient", "warmed_drought", "drought", "warmed"))

# just peak drought
data_16S_filtered_noinsect_peakdrought <- subset_samples(data_16S_filtered, Drought%in%c("Peak-Drought"))
summarize_phyloseq(data_16S_filtered_noinsect_peakdrought)

#####
data_16S_filtered_noinsect_peakdrought_wd <- subset_samples(data_16S_filtered, Drought%in%c("Peak-Drought", "Post-Drought"))

data_16S_filtered_noinsect_peakdrought_wd <- subset_samples(data_16S_filtered_noinsect_peakdrought_wd, Subplot_Descriptions%in%c("drought", "warmed"))

## Brandon's mess
otu_table <- t(as(data_16S_filtered_noinsect_peakdrought_wd@otu_table, "matrix"))
otu_table <- as.data.frame(otu_table)

library(hillR)
shan <- hill_taxa(otu_table, q=1)
shan <- merge(data_16S_filtered_noinsect_peakdrought_wd@sam_data, shan, by = 0)

shan %>%
        ggplot(aes(x=Drought, y = y, fill = Subplot_Descriptions)) +
        geom_boxplot()

mod <- lm(y ~ Drought * Subplot_Descriptions + 0, data = shan)
aov <- aov(y ~ Drought * Subplot_Descriptions, data = shan)
summary(aov)

# comparisons
emmeans(aov, list(pairwise ~ Drought*Subplot_Descriptions), adjust = "tukey")
mod_aov.emm <- emmeans(aov, ~ Drought*Subplot_Descriptions)
contrast(mod_aov.emm, "consec", simple = "each", combine = F, adjust = "mvt")
emmip(mod_aov.emm, Drought ~ Subplot_Descriptions)
...
plot_ordination(ps.rarefied, ordination, color="Subplot_Descriptions", shape = "Drought") + 
...
set.seed(22)
adonis2(wunifrac_dist ~ sample_data(ps.rarefied)$Subplot_Descriptions + sample_data(ps.rarefied)$Drought)
...
Beta Diversity - no insecticide plots and no irrigated control
```{r}
#  Rarefy the samples without replacement. Rarefaction is used to simulate even number of reads per sample. The rarefaction depth chosen is the 90% of the minimum sample depth in the dataset.
ps.rarefied_no_insect_noIRC = rarefy_even_depth(data_16S_filtered_no_insect_noIRC, rngseed=1, sample.size = 0.9*min(sample_sums(data_16S_filtered_no_insect_noIRC)), replace=F)

# PCoA plot using WEIGHTED UniFrac as distance
weighted_unifrac_dist_no_insect_noIRC = phyloseq::distance(ps.rarefied_no_insect_noIRC, method="unifrac", weighted=T)
ordination = ordinate(ps.rarefied_no_insect_noIRC, method="PCoA", distance=weighted_unifrac_dist_no_insect_noIRC)

#png("T7_warmx_16S_pcoa_weighted_unifrac_no_insects.png", units="in", width=10, height=6, res=300)
plot_ordination(ps.rarefied_no_insect_noIRC, ordination, color="Subplot_Descriptions", shape = "Drought") + 
        geom_point(aes(color = Subplot_Descriptions, shape = Drought), size=4, alpha=0.75) +
        #stat_ellipse() +
        scale_colour_manual(name = "Subplot Descriptions", 
                          breaks = c("irrigated_control", "ambient", "warmed", "drought", "warmed_drought"),
...
perm1 <- with(sample_data(ps.rarefied_no_insect_noIRC), how(blocks = Replicate, plots = Plots(strata = Footprint_Location, type = "free"), nperm = 9999))

# code wont work below - says its unbalanced
adonis2(weighted_unifrac_dist_no_insect_noIRC ~ sample_data(ps.rarefied_no_insect_noIRC)$Subplot_Descriptions*sample_data(ps.rarefied_no_insect_noIRC)$Drought, by = "margin", permutations = perm1)
...
PCoA excluding pre drought data
```{r}
ps.rarefied_nopredrought = rarefy_even_depth(data_16S_filtered_nopredrought, rngseed=1, sample.size = 0.9*min(sample_sums(data_16S_filtered_nopredrought)), replace=F)

# PCoA plot using WEIGHTED UniFrac as distance
weighted_unifrac_dist = phyloseq::distance(ps.rarefied_nopredrought, method="unifrac", weighted=T)
ordination = ordinate(ps.rarefied_nopredrought, method="PCoA", distance=weighted_unifrac_dist)

#png("T7_warmx_16S_pcoa_weighted_unifrac_no_insects.png", units="in", width=10, height=6, res=300)
plot_ordination(ps.rarefied_nopredrought, ordination, color="Subplot_Descriptions", shape = "Drought") + 
        geom_point(aes(color = Subplot_Descriptions, shape = Drought), size=4, alpha=0.75) +
        #stat_ellipse() +
        scale_colour_manual(name = "Subplot Descriptions", 
                          breaks = c("irrigated_control", "ambient", "warmed", "drought", "warmed_drought"),
...
# Test whether the treatments ("Subplot_Descriptions") differ significantly from each other using the permutational ANOVA (PERMANOVA) analysis:
set.seed(22)
adonis2(weighted_unifrac_dist ~ sample_data(ps.rarefied_nopredrought)$Subplot_Descriptions + sample_data(ps.rarefied_nopredrought)$Drought)
```
...
# DIFFERENTIAL ABUNDANCE - irrigated_control vs drought
```{r}
results1 <- results(diagdds, alpha = 0.01, contrast = c("Subplot_Descriptions", "irrigated_control", "drought"))
sigtab_results1 <- results1[which(results1$padj < 0.01), ]
#sigtab_results_ambient_vs_drought <- results[which(results$padj < 0.01), ]
summary(sigtab_results1)
...
# Order Phylum
x <- tapply(sigtab_results_TAX_genra$log2FoldChange, sigtab_results_TAX_genra$Phylum, function(x) max(x))
x <- sort(x, TRUE)
sigtab_results_TAX_genra$Class <- factor(as.character(sigtab_results_TAX_genra$Phylum), levels=names(x))
# Order Genus 
...
png("diff_abun_irrigated_drought.png", units="in", width=14, height=7, res=300)
ggplot(sigtab_results_TAX_genra1, aes(x=Class, y=log2FoldChange, color=Phylum)) + 
        geom_point(size=5) + 
        geom_hline(yintercept=0, linewidth=1) +
        annotate("text", x = 30, y=15, label = "Irrigated Control", size=4) +
        annotate("text", x = 30, y=-5, label = "Drought", size=4) +
        scale_colour_manual(values = c("firebrick4","tomato", "olivedrab1", "orange1" ,"deepskyblue", "khaki1",
                                     "forestgreen", "darkorange3", "midnightblue", "powderblue", "mediumorchid4", "aquamarine", "deeppink", "gold", "mediumblue", "salmon", "goldenrod4", "darkseagreen", "gray0", "darkslategrey", "firebrick2")) +
        theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5))
dev.off()
# 50 OTUs are enriched in irrigated control (adj p-value < 0.01)
# 41 OTUs are enriched in drought (adj p-value < 0.01)
```

# DIFFERENTIAL ABUNDANCE - irrigated control vs warmed
```{r}
results2 <- results(diagdds, alpha = 0.01, contrast = c("Subplot_Descriptions", "irrigated_control", "warmed"))
sigtab_results2 <- results2[which(results2$padj < 0.01), ]
#sigtab_results_ambient_vs_warmed <- results[which(results$padj < 0.01), ]
summary(sigtab_results2)
...
png("diff_abun_irrigated_warmed.png", units="in", width=14, height=7, res=300)
ggplot(sigtab_results_TAX_genra2, aes(x=Class, y=log2FoldChange, color=Phylum)) + 
        geom_point(size=3) + 
        geom_hline(yintercept=0, linewidth=1) +
        annotate("text", x = 25, y=10, label = "Irrigated Control", size=4) +
        annotate("text", x = 25, y=-5, label = "Warmed", size=4) +
        scale_colour_manual(values = c("firebrick4","tomato", "olivedrab1", "orange1" ,"deepskyblue", "khaki1",
        ...
        theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5))
dev.off()
# 62 OTUs are enriched in irrigated control (adj p-value < 0.01)
# 34 OTUs are enriched in warmed (adj p-value < 0.01)
```

# DIFFERENTIAL ABUNDANCE - irrigated control vs warmed + drought
```{r}
results3 <- results(diagdds, alpha = 0.01, contrast = c("Subplot_Descriptions", "irrigated_control", "warmed_drought"))
sigtab_results3 <- results3[which(results3$padj < 0.01), ]
summary(sigtab_results3)
#sigtab_results_ambient_vs_warmed <- results[which(results$padj < 0.01), ]
...
png("diff_abun_irrigated_warmed_drought.png", units="in", width=14, height=7, res=300)
ggplot(sigtab_results_TAX_genra3, aes(x=Class, y=log2FoldChange, color=Phylum)) + 
        geom_point(size=3) + 
        geom_hline(yintercept=0, linewidth=1) +
        annotate("text", x = 30, y=15, label = "Irrigated Control", size=4) +
        annotate("text", x = 30, y=-5, label = "Warmed + Drought", size=4) +
        scale_colour_manual(values = c("firebrick4","tomato", "olivedrab1", "orange1" ,"deepskyblue", "khaki1",
                                     "forestgreen", "darkorange3", "midnightblue", "powderblue", "mediumorchid4", "aquamarine", "deeppink", "gold", "mediumblue", "salmon", "goldenrod4", "darkseagreen", "gray0", "darkslategrey", "firebrick2", "springgreen", "thistle1", "turquoise4")) +
        theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5))
dev.off()
# 52 OTUs are enriched in irrigated control (adj p-value < 0.01)
# 48 OTUs are enriched in warmed + drought (adj p-value < 0.01)
```
