...
author: "Moriah Young"
date: "2023-01-31, updated 2024-04-29"
output: pdf_document
editor_options: 
...
physeq_tree <- qza_to_phyloseq(
        tree = "/Users/moriahyoung/Desktop/phyloseq_object/16S-2021-fasttree-rooted-tree.qza"
        )
physeq_tree
```
...
taxonomy <- read.csv("/Users/moriahyoung/Desktop/phyloseq_object/16S-2021-taxonomy-filtered-noncontam.csv")
otu <- read.csv("/Users/moriahyoung/Desktop/phyloseq_object/16S-2021-table-filtered-noncontam.csv")
metadata <- read.csv("/Users/moriahyoung/Desktop/phyloseq_object/bacterial_2021_metadata.csv")

# Create phyloseq objects of filtered data (16S)
data_16S_filtered <- read_csv2phyloseq(otu.file = "/Users/moriahyoung/Desktop/phyloseq_object/16S-2021-table-filtered-noncontam.csv",
                                       taxonomy.file = "/Users/moriahyoung/Desktop/phyloseq_object/16S-2021-taxonomy-filtered-noncontam.csv",
...
sum3 <- abundant_16S_phylum_melt %>% 
  group_by(Drought, sampling_period, Phylum) %>%
  summarise(means = mean(Abundance))
...
ggplot(sum5, aes(x = sampling_period, y = means, fill = Class)) +
        geom_bar(position = "stack", stat = "identity", col = "black") +
...
sum3 <- abundant_16S_family_melt %>% 
  group_by(Subplot_Descriptions, sampling_period, Family) %>%
  summarise(means = mean(Abundance))
...
# facet_grid by treatment aka "Subplot_Descriptions"
#png("T7_warmx_16S_family_stacked_bar_plot_by_treatment.png", units="in", width=10, height=6, res=300)
ggplot(sum3_noinsect, aes(x = sampling_period, y = means, fill = Family)) +
        geom_bar(position = "stack", stat = "identity", col = "black") +
        facet_grid(~Subplot_Descriptions, scale="free", labeller = as_labeller(treatment_names)) +
        theme(legend.position = "right", legend.title = element_blank(), axis.line = element_blank()) +
        #scale_fill_manual(values = c("firebrick4","tomato", "darkorange3", "orange1" ,"goldenrod4", "khaki1",
...
# facet_grid by treatment aka "Subplot_Descriptions"
#png("T7_warmx_16S_genus_stacked_bar_plot_by_treatment.png", units="in", width=11, height=6, res=300)
ggplot(sum4_noinsect, aes(x = sampling_period, y = means, fill = Genus)) +
        geom_bar(position = "stack", stat = "identity", col = "black") +
        facet_grid(~Subplot_Descriptions, scale="free", labeller = as_labeller(treatment_names)) +
        theme(legend.position = "right", legend.title = element_blank(), axis.line = element_blank()) +
        #scale_fill_manual(values = c("firebrick4","tomato", "darkorange3", "orange1" ,"goldenrod4", "khaki1",
...
sample_data(rare_16S_noinsect)$sample_period <- ordered(sample_data(rare_16S_noinsect)$sampling_period, c("Pre-Drought", "Peak-Drought", "Post-Drought", "Recovery"))
...
plot_richness(rare_16S_noinsect, x="sampling_period", color="Subplot_Descriptions", measures=c("Observed"))
plot_richness(rare_16S_noinsect, x="sampling_period", measures=c("Observed", "Shannon")) + geom_boxplot()
plot_richness(rare_16S_noinsect, x="Subplot_Descriptions", measures=c("Observed")) + geom_boxplot() + facet_grid(.~sampling_period)
plot_richness(rare_16S_noinsect, x="Subplot_Descriptions", measures=c("Shannon")) + geom_boxplot() + facet_grid(.~sampling_period)
...
plot_richness(rare_16S_noinsect, x="sampling_period", measures=c("Shannon")) + 
        geom_boxplot(aes(fill = Subplot_Descriptions)) + 
...
kruskal.test(richness$Shannon ~ sample_data(rare_16S_noinsect)$sampling_period)
...
#png("T7_warmx_16S_pcoa_no_insects.png", units="in", width=10, height=6, res=300)
plot_ordination(ps.rarefied, ordination, color="Subplot_Descriptions", shape = "sampling_period") + 
        geom_point(aes(color = Subplot_Descriptions, shape = sample_period), size=4, alpha=0.75) +
        #stat_ellipse(aes(fill = Drought), geom = "polygon", alpha = .5) +
        scale_colour_manual(name = "Subplot Descriptions", 
...
weighted_unifrac_dist <- phyloseq::distance(ps.rarefied, method="unifrac", weighted=T)
ordination <- ordinate(ps.rarefied, method="PCoA", distance=weighted_unifrac_dist)
...
# Test whether the treatments ("Subplot_Descriptions") differ significantly from each other using the permutational ANOVA (PERMANOVA) analysis:
set.seed(22)
adonis2(wunifrac_dist ~ sample_data(ps.rarefied)$Subplot_Descriptions + sample_data(ps.rarefied)$sampling_period)

# Test whether the treatments ("Subplot_Descriptions") differ significantly from each other using the permutational ANOVA (PERMANOVA) analysis:
set.seed(22)
adonis2(weighted_unifrac_dist ~ sample_data(ps.rarefied)$Subplot_Descriptions + sample_data(ps.rarefied)$sampling_period)
...
perm1 <- with(sample_data(ps.rarefied), how(blocks = Replicate, plots = Plots(strata=Footprint, type="free"), within=Within(type="free"), nperm = 9999))

# no footprint
perm2 <- with(sample_data(ps.rarefied), how(blocks = Replicate, within=Within(type="free"), nperm = 9999))

adonis2(wunifrac_dist ~ sample_data(ps.rarefied)$Subplot_Descriptions * sample_data(ps.rarefied)$sampling_period, by = "margin", permutations = perm1)

adonis2(wunifrac_dist ~ sample_data(ps.rarefied)$Subplot_Descriptions + sample_data(ps.rarefied)$sampling_period, by = "margin", permutations = perm2)

adonis2(wunifrac_dist ~ sample_data(ps.rarefied)$Drought + sample_data(ps.rarefied)$Warming + sample_data(ps.rarefied)$Insecticide + sample_data(ps.rarefied)$sampling_period, by = "margin", permutations = perm2)

adonis2(wunifrac_dist ~ sample_data(ps.rarefied)$Drought + sample_data(ps.rarefied)$Warming + sample_data(ps.rarefied)$sampling_period, by = "margin", permutations = perm2)

# if you need to change a phyloseq "slot" to a factor:
# Access sample data slot
sample_data <- sample_data(ps.rarefied)
...
sample_data(ps.rarefied) <- sample_data
...
# PCoA aggregating by different levels
```{r}
data_16S_filtered_balanced
# class
class <- aggregate_taxa(data_16S_filtered_balanced, 'Class', verbose = FALSE)

# PCoA plot using bray-curtis as distance
bray_dist = phyloseq::distance(class, method="bray")
ordination = ordinate(class, method="PCoA", distance=bray_dist)

plot_ordination(class, ordination) + 
        geom_point(aes(color = sampling_period), size=4, alpha=0.75) +
        #stat_ellipse() +
        theme(aspect.ratio=1)

set.seed(22)
adonis2(bray_dist ~ sample_data(class)$Subplot_Descriptions + sample_data(class)$sampling_period)

# this works
perm1 <- with(sample_data(class), how(blocks=Rep, plots=Plots(strata=Rep_Footprint, type="free"), within=Within(type="free"), nperm = 999))

perm2 <- with(sample_data(class), how(blocks=Rep, within=Within(type="free"), nperm = 999))

adonis2(bray_dist ~ sample_data(class)$Subplot_Descriptions + sample_data(class)$sampling_period, by = "margin", permutations = perm1)

adonis2(bray_dist ~ sample_data(class)$Subplot_Descriptions + sample_data(class)$sampling_period, by = "margin", permutations = perm2)

adonis2(bray_dist ~ sample_data(class)$Drought + sample_data(class)$Warming + sample_data(class)$sampling_period, by = "margin", permutations = perm2)
...
phylum <- aggregate_taxa(data_16S_filtered_balanced, 'Phylum', verbose = FALSE)

# PCoA plot using bray-curtis as distance
bray_dist = phyloseq::distance(phylum, method="bray")
ordination = ordinate(phylum, method="PCoA", distance=bray_dist)

plot_ordination(phylum, ordination) + 
        geom_point(aes(color = sampling_period), size=4, alpha=0.75) +
        theme(aspect.ratio=1)

set.seed(22)
adonis2(bray_dist ~ sample_data(phylum)$Subplot_Descriptions + sample_data(phylum)$sampling_period)
adonis2(bray_dist ~ sample_data(phylum)$Drought + sample_data(phylum)$Warming + sample_data(phylum)$sampling_period)

# this works
perm1 <- with(sample_data(phylum), how(blocks=Rep, plots=Plots(strata=Rep_Footprint, type="free"), within=Within(type="free"), nperm = 999))

perm2 <- with(sample_data(phylum), how(blocks=Rep, within=Within(type="free"), nperm = 999))

adonis2(bray_dist ~ sample_data(phylum)$Subplot_Descriptions + sample_data(phylum)$sampling_period, by = "margin", permutations = perm1)

adonis2(bray_dist ~ sample_data(phylum)$Drought + sample_data(phylum)$Warming + sample_data(phylum)$sampling_period, by = "margin", permutations = perm1)

adonis2(bray_dist ~ sample_data(phylum)$Drought + sample_data(phylum)$Warming + sample_data(phylum)$sampling_period, by = "margin", permutations = perm2)

rda.test = ordinate(phylum, method="RDA")
plot_ordination(phylum, rda.test, color="Drought", shape = "sampling_period") + 
        geom_point(aes(shape = sampling_period), size=4, alpha=0.75) +
        stat_ellipse(aes(group = Drought)) +
        theme(aspect.ratio=1)
```

No irrigated control data
```{r}
data_16S_noIRC

# phylum

phylum <- aggregate_taxa(data_16S_noIRC, 'Phylum', verbose = FALSE)
...
plot_ordination(phylum, ordination) + 
        geom_point(aes(color = Subplot_Descriptions), size=4, alpha=0.75) +
        #stat_ellipse(aes(group = Subplot_Descriptions)) +
        theme(aspect.ratio=1)

set.seed(22)
adonis2(bray_dist ~ sample_data(phylum)$Subplot_Descriptions + sample_data(phylum)$sampling_period)
adonis2(bray_dist ~ sample_data(phylum)$Drought + sample_data(phylum)$Warming + sample_data(phylum)$Insecticide + sample_data(phylum)$sampling_period)
...
adonis2(bray_dist ~ sample_data(phylum)$Subplot_Descriptions + sample_data(phylum)$sampling_period, by = "margin", permutations = perm2)

adonis2(bray_dist ~ sample_data(phylum)$Drought + sample_data(phylum)$Warming + sample_data(phylum)$Insecticide + sample_data(phylum)$sampling_period, by = "margin", permutations = perm1)

adonis2(bray_dist ~ sample_data(phylum)$Drought + sample_data(phylum)$Warming + sample_data(phylum)$Insecticide + sample_data(phylum)$sampling_period, by = "margin", permutations = perm2)

rda.test = ordinate(phylum, method="RDA")
plot_ordination(phylum, rda.test, color="Insecticide") + 
        geom_point(aes(shape = sampling_period), size=4, alpha=0.75) +
        stat_ellipse(aes(group = Insecticide)) +
        theme(aspect.ratio=1)
```
...
RDA
```{r}
rda_rare_16S = ordinate(rare_16S, method="RDA")

plot_ordination(rare_16S, rda_rare_16S, color="Subplot_Descriptions", shape = "sampling_period") + 
        geom_point(aes(color = Subplot_Descriptions, shape = sampling_period), size=4, alpha=0.75) +
        #stat_ellipse() +
        scale_colour_manual(name = "Subplot Descriptions", 
                          breaks = c("irrigated_control", "ambient", "warmed", "drought", "warmed_drought"),
