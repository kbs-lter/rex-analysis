...
Create phyloseq object that doesn't include insecticide treatments
```{r}
...
#bray.dist <- phyloseq::distance(T2, method = "bray”)

set.seed(53) #you can pick any number, but this prevents the results from being different every time. You need to run this every time before you do an adonis command.
...
# NMDS - Ordination using Phyloseq package
```{r}
ord_16S <- prune_taxa(names(sort(taxa_sums(ps.rarefied), TRUE)[1:50]), ps.rarefied)

# jaccard ordination
jaccard_pcoa_16S <- ordinate(
        physeq = ord_16S, 
        method = "NMDS", 
        distance = "jaccard"
        )
...
# plot ordination
plot_ordination(ord_16S, jaccard_pcoa_16S, "samples")
#png("T7_warmx_16S_NMDS_ordination.png", units="in", width=10, height=6, res=300)
plot_ordination(
  physeq = ord_16S,                                                         #phyloseq object
  ordination = jaccard_pcoa_16S) +                                                #ordination
  geom_point(aes(fill = Subplot_Descriptions, shape = Drought), size = 3) + #sets fill color to sampletype
        ...
        scale_fill_discrete(name = "Subplot Descriptions", 
                            breaks = c("irrigated_control", "ambient", "warmed", "drought", "warmed_drought"),
                            labels = c("Irrigated Control", "Ambient", "Warmed", "Drought", "Warmed + Drought")) +
...
Code below is very much a work in progress!!
ASV differential abundance testing with DESeq2

I want to know the differential abundances between warming and drought treatments (to start off with)
```{r}
# Convert phyloseq data to DESeq2 dds (DESeq2 Data Set?) object
# Note - cannot use phyloseq object that has sample data ordered or the line of code will not work
# Re line 102
dds <- phyloseq_to_deseq2(data_16S_filtered_noinsect, ~ Subplot_Descriptions + Drought)
dds <- DESeq(dds)
alpha = 0.01
res = results(dds)
...
ggplot(res_sig, aes(x=Genus, y=log2FoldChange, color=Phylum)) + geom_point(size=3) + 
...
res <- results(dds, tidy=TRUE, contrast=c("Subplot_Descriptions", "warmed", "drought")) %>%
  arrange(padj, pvalue) %>%
  tbl_df()
