...
```{r}
#remove hour 0
standardno0 <- standardsamples %>% filter(!Delta %in% c("0"))

# checks what type of distribution your data matches
descdist(standardno0$Delta, discrete = FALSE)
# should be a bell curve if normal
hist(standardno0$Delta) #zero reads throw off graph but appears normal with them
# should be a straight diagonal line if normal
qqnorm(standardno0$Delta) #skewed
#sample size too big for Shapiro test

#still very skewed even without zero reads
```
...
m1 <- lmer(Richness168hr ~ Subplot_Descriptions + (1|Rep/Footprint_Location/Subplot), data = biologHr168diversity, REML=F)
summary(m1)
```
