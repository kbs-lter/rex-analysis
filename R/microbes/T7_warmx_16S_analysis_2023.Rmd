...
metadata <- rownames_to_column(data.frame(data_rare_drought@sam_data), "SampleID")
vectors_metadata <- dplyr::inner_join(vectors, metadata, by = "SampleID")

ggplot(vectors_metadata, aes(x = Axis.1, y = Axis.2, color = Date)) + geom_point()
...
metadata <- rownames_to_column(data.frame(data_rare_warm_drought@sam_data), "SampleID")
vectors_metadata <- dplyr::inner_join(vectors, metadata, by = "SampleID")
